    ...
    private void UpdateForce()
    {
        ...
        for (int i = 0; i < positions.Count; i++) {
            ...
            if (TurbulenceStrength > 0.000001f) {
                ...
                turbulenceVel.x += ((Mathf.PerlinNoise(pos.z, pos.y) * 1.2f - 1) * Amplitude) * Time.deltaTime * TurbulenceStrength / 10f;
               // turbulenceVel.y += ((Mathf.PerlinNoise(pos.x, pos.z) * 1 - 1) * Amplitude) * Time.deltaTime * TurbulenceStrength / 10f;
                turbulenceVel.z += ((Mathf.PerlinNoise(pos.y, pos.x) * 1.2f - 1) * Amplitude) * Time.deltaTime * TurbulenceStrength / 10f;
            }
