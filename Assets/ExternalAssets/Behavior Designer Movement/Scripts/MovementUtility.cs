...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
    public static class MovementUtility
    {
        ...
        public static GameObject WithinSight(Transform transform, Vector3 positionOffset, float fieldOfViewAngle,
            ...
            LayerMask ignoreLayerMask, bool useTargetBone, HumanBodyBones targetBone, bool drawDebugRay)
        {
            ...
            if (hitCount > 0)
            {
                ...
                for (int i = 0; i < hitCount; ++i)
                {
                    ...
                    if ((obj = WithinSight(transform, positionOffset, fieldOfViewAngle, viewDistance,
                            ...
                            useTargetBone, targetBone, drawDebugRay)) != null)
                    {
        ...
        public static GameObject WithinSight2D(Transform transform, Vector3 positionOffset, float fieldOfViewAngle,
            ...
            float angleOffset2D, LayerMask ignoreLayerMask, bool drawDebugRay)
        {
            GameObject objectFound = null;
            var hitCount =
                Physics2D.OverlapCircleNonAlloc(transform.position, viewDistance, overlapColliders, objectLayerMask);
            if (hitCount > 0)
            {
#if UNITY_EDITOR
                if (hitCount == overlapColliders.Length)
                ...
                for (int i = 0; i < hitCount; ++i)
                {
                    ...
                    if ((obj = WithinSight(transform, positionOffset, fieldOfViewAngle, viewDistance,
                            ...
                            ignoreLayerMask, false, HumanBodyBones.Hips, drawDebugRay)) != null)
                    {
                        // This object is within sight. Set it to the objectFound GameObject if the angle is less than any of the other objects
                        if (angle < minAngle)
                        {
        ...
        public static GameObject WithinSight(Transform transform, Vector3 positionOffset, float fieldOfViewAngle,
            ...
            bool useTargetBone, HumanBodyBones targetBone, bool drawDebugRay)
        {
            float angle;
            return WithinSight(transform, positionOffset, fieldOfViewAngle, viewDistance, targetObject, targetOffset,
                false, 0, out angle, ignoreLayerMask, useTargetBone, targetBone, drawDebugRay);
        }

        // Public helper function that will automatically create an angle variable that is not used. This function is useful if the calling object doesn't
        // care about the angle between transform and targetObject
        public static GameObject WithinSight2D(Transform transform, Vector3 positionOffset, float fieldOfViewAngle,
            float viewDistance, GameObject targetObject, Vector3 targetOffset, float angleOffset2D,
            LayerMask ignoreLayerMask, bool useTargetBone, HumanBodyBones targetBone, bool drawDebugRay)
        {
            float angle;
            return WithinSight(transform, positionOffset, fieldOfViewAngle, viewDistance, targetObject, targetOffset,
                true, angleOffset2D, out angle, ignoreLayerMask, useTargetBone, targetBone, drawDebugRay);
        }
        ...
        public static GameObject WithinSight(Transform transform, Vector3 positionOffset, float fieldOfViewAngle,
            ...
            out float angle, int ignoreLayerMask, bool useTargetBone, HumanBodyBones targetBone, bool drawDebugRay)
        {
            ...
            if (direction.magnitude < viewDistance && angle < fieldOfViewAngle * 0.5f)
            {
                ...
                var hitTransform = LineOfSight(transform, positionOffset, targetObject, targetOffset, usePhysics2D,
                ...
                if (hitTransform != null)
                {
                    ...
                    else
                    {
                        if (drawDebugRay)
                        {
                         ...
                         GetComponentForType<Collider2D>(targetObject) == null)
                {
        ...
        public static Transform LineOfSight(Transform transform, Vector3 positionOffset, GameObject targetObject,
            Vector3 targetOffset, bool usePhysics2D, int ignoreLayerMask, bool drawDebugRay)
        {
        ...
        public static GameObject WithinHearingRange(Transform transform, Vector3 positionOffset,
            float audibilityThreshold, float hearingRadius, Collider[] overlapColliders, LayerMask objectLayerMask)
        {
        ...
        {
            GameObject objectHeard = null;
            var hitCount = Physics2D.OverlapCircleNonAlloc(transform.TransformPoint(positionOffset), hearingRadius,
                overlapColliders, objectLayerMask);
            if (hitCount > 0)
            {
#if UNITY_EDITOR
                if (hitCount == overlapColliders.Length)
            ...
            if ((colliderAudioSource = GetComponentsForType<AudioSource>(targetObject)) != null)
            {
                for (int i = 0; i < colliderAudioSource.Length; ++i)
                {
                    if (colliderAudioSource[i].isPlaying)
                    {
                        ...
                        {
                            // linear
                            audibility = colliderAudioSource[i].volume * Mathf.Clamp01(
                                (distance - colliderAudioSource[i].minDistance) / (colliderAudioSource[i].maxDistance -
