...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
    ...
    public class CanSeeObject : Conditional
    {
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            if (usePhysics2D)
            {
                if (targetObjects.Value != null && targetObjects.Value.Count > 0)
                {
                    ...
                    for (int i = 0; i < targetObjects.Value.Count; ++i)
                    {
                        ...
                        if ((obj = MovementUtility.WithinSight(transform, offset.Value, fieldOfViewAngle.Value,
                                viewDistance.Value, targetObjects.Value[i], targetOffset.Value, true,
                                angleOffset2D.Value, out angle, ignoreLayerMask, useTargetBone.Value, targetBone,
                                drawDebugRay.Value)) != null)
                        {
                ...
                else if (targetObject.Value != null)
                {
                    ...
                    returnedObject.Value = MovementUtility.WithinSight2D(transform, offset.Value,
                        fieldOfViewAngle.Value, viewDistance.Value, targetObject.Value, targetOffset.Value,
                        angleOffset2D.Value, ignoreLayerMask, useTargetBone.Value, targetBone, drawDebugRay.Value);
                }
                else if (!string.IsNullOrEmpty(targetTag.Value))
                {
                    ...
                    for (int i = 0; i < targets.Length; ++i)
                    {
                        ...
                        GameObject obj;
                        if ((obj = MovementUtility.WithinSight(transform, offset.Value, fieldOfViewAngle.Value,
                                viewDistance.Value, targets[i], targetOffset.Value, true, angleOffset2D.Value,
                ...
                else
                {
                    ...
                    returnedObject.Value = MovementUtility.WithinSight2D(transform, offset.Value,
                        fieldOfViewAngle.Value, viewDistance.Value, overlap2DColliders, objectLayerMask,
                        targetOffset.Value, angleOffset2D.Value, ignoreLayerMask, drawDebugRay.Value);
                }
            }
            else
            {
                ...
                else if (targetObject.Value != null)
                {
                    ...
                    returnedObject.Value = MovementUtility.WithinSight(transform, offset.Value, fieldOfViewAngle.Value,
                        viewDistance.Value, targetObject.Value, targetOffset.Value, ignoreLayerMask,
                ...
                else if (!string.IsNullOrEmpty(targetTag.Value))
                {
                    ...
                    for (int i = 0; i < targets.Length; ++i)
                    {
                        ...
                        GameObject obj;
                        if ((obj = MovementUtility.WithinSight(transform, offset.Value, fieldOfViewAngle.Value,
                                viewDistance.Value, targets[i], targetOffset.Value, false, angleOffset2D.Value,
                                out angle, ignoreLayerMask, useTargetBone.Value, targetBone, drawDebugRay.Value)) !=
                            null)
                        {
                            // This object is within sight. Set it to the objectFound GameObject if the angle is less than any of the other objects
        ...
        {
            MovementUtility.DrawLineOfSight(Owner.transform, offset.Value, fieldOfViewAngle.Value, angleOffset2D.Value,
                viewDistance.Value, usePhysics2D);
        }
